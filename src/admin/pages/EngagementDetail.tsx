...
function SetContractValueForm({ engagement, onSaved }: { engagement: Engagement; onSaved: () => void }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-sm font-semibold text-slate-300">Set Contract Value</h2>
            ...
            {error && <p className="mt-3 text-sm text-red-400">Something went wrong — please try again.</p>}
...
function SprintsForm({ engagement, onSaved }: { engagement: Engagement; onSaved: () => void }) {
    ...
    return (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-sm font-semibold text-slate-300">Define Sprint 1 &amp; Start Project</h2>
            <p className="mt-1 text-sm text-slate-500">
            ...
            <button type="button" onClick={addPhase} className="mt-3 text-sm font-medium text-blue-400 hover:text-blue-300">
                + Add Phase
            </button>
            {error && <p className="mt-3 text-sm text-red-400">Something went wrong — please try again.</p>}
            <div className="mt-4 flex justify-end">
...
function SprintsPanel({ engagement, isAdmin, onSaved }: { engagement: Engagement; isAdmin: boolean; onSaved: () => void }) {
    ...
    return (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-sm font-semibold text-slate-300">Sprints</h2>
            ...
            <div className="mt-4 space-y-6">
                {engagement.sprints.map((sprint, sprintIndex) => (
                    <div key={sprintIndex} className={sprintIndex > 0 ? 'border-t border-slate-800 pt-6' : ''}>
                        ...
                        <ol className="mt-3 space-y-2">
                            {sprint.phases.map((phase, phaseIndex) => (
                                <li key={phaseIndex} className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className={`flex-1 ${phase.status !== 'not_started' ? 'font-medium text-white' : 'text-slate-400'}`}>
                                        {phase.name}
                                    </span>
                                    {isAdmin ? (
                                        <select
                                            ...
                                            disabled={saving}
                                            className="shrink-0 rounded-lg border border-slate-700 bg-slate-950/60 px-2 py-1.5 text-xs text-slate-300 disabled:opacity-60"
                                        >
                        ...
                        {isAdmin && (
                            <div className="mt-3 flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="shrink-0 rounded-lg border border-slate-700 px-4 py-2 text-xs font-medium text-slate-300 hover:border-slate-600 disabled:opacity-60"
                                >
                                    Add Phase
                                </button>
                            </div>
...
function DeliverAction({
    ...
    return (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-sm font-semibold text-slate-300">Delivery</h2>
            <p className="mt-1 text-sm text-slate-500">
                {finalVerified
...
function AssignedDevelopers({ engagement, onSaved }: { engagement: Engagement; onSaved: () => void }) {
    ...
    return (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-sm font-semibold text-slate-300">Assigned Developers</h2>
            {developers.length === 0 ? (
    ...
    return (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-6">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold capitalize text-slate-300">{invoice.type} Invoice</h3>
            ...
            <div className="mt-4 flex justify-end gap-2">
                <button
                    ...
                    disabled={exporting}
                    className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-medium text-slate-300 hover:border-slate-600 disabled:opacity-60"
                >
    ...
    if (engagement === null) {
        return (
            <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-10 text-center">
                ...
                <Link to="/admin/engagements" className="mt-3 inline-block text-sm text-blue-400 hover:text-blue-300">
                    Back to Engagements
                </Link>
            </div>
        )
        ...
        <div>
            ...
            <div className="mt-3 flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-400">{engagement.customerEmail}</p>
                </div>
                <StatusBadge status={engagementStatusLabels[engagement.status]} />
            </div>
            {engagement.description && <p className="mt-4 max-w-2xl text-sm text-slate-400">{engagement.description}</p>}
