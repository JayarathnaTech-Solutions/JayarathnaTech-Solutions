...
export function ChatThread({
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white p-6">
            <h2 className="text-sm font-semibold text-slate-600">Team Chat</h2>
                ...
                {messages === null ? (
                    ...
                    messages.map((message) => (
                        <div key={message.id} className={message.senderId === senderId ? 'text-right' : 'text-left'}>
                            ...
                            <p className="mt-1 text-xs text-slate-400">{formatTime(message.createdAt)}</p>
                        </div>
                    ))
                )}
                <div ref={bottomRef} />
            ...
            <form onSubmit={handleSend} className="mt-4 flex items-center gap-2">
                <label className="shrink-0 cursor-pointer text-slate-500 hover:text-slate-900">
