        ...
        <div className="space-y-3">
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6">
                <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold capitalize text-slate-600 dark:text-slate-300">{invoice.type} Payment</h3>
...
export function PortalEngagementDetail() {
    ...
    if (engagement === null) {
        return (
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-10 text-center">
                ...
                <Link to="/portal" className="mt-3 inline-block text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300">
                    Back to Dashboard
                </Link>
            </div>
        )
    ...
    return (
        <div>
            <Link to="/portal" className="text-sm text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
                Your Projects
            </Link>
            ...
            {engagement.description && <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">{engagement.description}</p>}

            {engagement.status === 'delivered' && (
                <div className="mt-6 rounded-xl border border-emerald-200 dark:border-emerald-800/50 bg-emerald-500/10 p-6 text-sm text-emerald-700 dark:text-emerald-300">
                    This project has been delivered. Thank you for working with us!
                </div>
            )}

            {engagement.status === 'pending_advance' && (
                <div className="mt-6 rounded-xl border border-amber-200 dark:border-amber-800/50 bg-amber-500/10 p-6 text-sm text-amber-700 dark:text-amber-300">
                    Your project will begin once the advance payment below is verified.
                </div>
            )}

            {engagement.sprints.length > 0 && (
                <div className="mt-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6">
                    <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Progress</h2>
                                ...
                                <ol className="mt-2 space-y-2">
                                    {sprint.phases.map((phase, phaseIndex) => (
                                        <li key={phaseIndex} className="flex items-center gap-3 text-sm">
                                            ...
                                            <span className={phase.status !== 'not_started' ? 'font-medium text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}>
                                                {phase.name}
            ...
            {(advanceInvoice || (finalInvoice && showFinalInvoice)) && (
                <div className="mt-6 space-y-6">
                    <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Payments</h2>
