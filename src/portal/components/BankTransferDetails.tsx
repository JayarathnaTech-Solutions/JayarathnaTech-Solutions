...
export function BankTransferDetails({ invoice, onSubmitted }: { invoice: Invoice; onSubmitted: () => void }) {
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white p-6">
            <h3 className="text-sm font-semibold text-slate-600">Bank Transfer Details</h3>
            {bankDetails === undefined ? (
                ...
                <dl className="mt-3 space-y-1.5 text-sm">
                    <div className="flex justify-between">
                        ...
                        <dd className="text-slate-600">{bankDetails.bankName}</dd>
                    </div>
                    <div className="flex justify-between">
                        <dt className="text-slate-500">Account Name</dt>
                        <dd className="text-slate-600">{bankDetails.accountName}</dd>
                    </div>
                    <div className="flex justify-between">
                        <dt className="text-slate-500">Account Number</dt>
                        <dd className="text-slate-600">{bankDetails.accountNumber}</dd>
                    </div>
                    <div className="flex justify-between">
                        <dt className="text-slate-500">Branch / SWIFT</dt>
                        <dd className="text-slate-600">{bankDetails.branchSwift}</dd>
                    </div>
                </dl>
            )}

            <div className="mt-5 border-t border-slate-200 pt-4">
                <label htmlFor={`proof-${invoice.id}`} className="mb-1.5 block text-sm font-medium text-slate-600">
                ...
                <input
                    ...
                    disabled={uploading}
                    className="block w-full text-sm text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-blue-600 file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-blue-500"
                />
                ...
                {invoice.status === 'proof_submitted' && (
                    <p className="mt-2 text-sm text-amber-600">Proof submitted — awaiting verification.</p>
                )}
            </div>
        </div>
    )
